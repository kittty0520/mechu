...
function displayResultFood() {
    if (indexArr.length === 0) {
        ...
        backButton.textContent = '테스트 다시하기';
        backButton.onclick = () => {
            const backquest = document.getElementById('quest');
            backquest.scrollIntoView({ behavior: 'smooth' });
        };
        document.getElementById('btn').appendChild(backButton);
        return null;
    }
...
function setBackButton(buttonElement) {
    const backButton = document.createElement('button');
    backButton.textContent = '테스트 돌아가기';
    backButton.addEventListener('click', () => {
        const backquest = document.getElementById('quest');
        backquest.scrollIntoView({ behavior: 'smooth' });
    });
    buttonElement.appendChild(backButton);
    document.getElementById('btn_re').style.display = 'none';
}
...
// 다시보기 버튼을 클릭하면 restart()함수를 실행하여 결과값 재출력
document.getElementById('btn_re').addEventListener('click', restart);
